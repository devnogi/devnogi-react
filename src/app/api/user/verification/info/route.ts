...
const logger = createLogger("API/user/verification/info");

export async function GET(request: NextRequest) {
  logger.info("========== GET /api/user/verification/info 요청 시작 ==========");

  try {
    const axios = createAuthServerAxios(request);
    const { data, status } = await axios.get(`${USER_VERIFICATION_ENDPOINT}/info`);

    logger.info(`Gateway 응답: status=${status}`, data);
    logger.info("========== GET /api/user/verification/info 요청 완료 ==========");

    return NextResponse.json(data, { status });
  } catch (error: unknown) {
    logger.error("GET /api/user/verification/info 에러 발생");
